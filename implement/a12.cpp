...
Node *ddl_insert(Node *entry,Node *temp)
{
    //entry && temp is the order to be inserted  in linked list
...
Collection insert_a(Collection c,int age)
{
    ...
    while(utemp!=NULL) //check again
    {
        ...
        while(temp!=NULL)
        {
            if(age<temp->age||temp->right==NULL)
            {
                cout<<"HELLO"<<temp->age<<endl;
                if(temp->bottom==NULL)
                {
            ...
            else if(age>temp->age) temp=temp->right;
            else
            {
                cout<<"Invalid entry already exists"<<endl;
                delete entry;
                return c;
            }
        }
